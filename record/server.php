<?php
  ...
  // unset($_SESSION['seat']);
	...
	function getId($username){
		global $db;
	  $query = 'SELECT `id` FROM `handler` WHERE `username` = "'.$username.'"';
  ...
  function ifExists($username){
	...
	if (isset($_POST['login_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT * FROM handler WHERE username='$username' AND password='$password'";
			...
			if (mysqli_num_rows($results) == 1) {
				...
				$_SESSION['userId'] = getId($username);
				$_SESSION['success'] = "You are now logged in";
				header('location: handler/index.php');
			}else {
